...
export function parseCallback(value: string, state: string, startedAt: number) {
  ...
  if (
    !value.startsWith(`${redirectUri}?`) ||
    value.includes('#') ||
    Date.now() - startedAt >= 600000
  ) {
    throw new Error('Tentativa inválida ou expirada. Entre novamente.');
  }
  const params = new URLSearchParams(value.slice(redirectUri.length + 1));
  if (
    [...params.keys()].some(
      (key) =>
        !['state', 'code', 'error'].includes(key) ||
  ...
  if (params.has('error')) {
    const messages: Record<string, string> = {
      ...
      attempt_expired: 'Tentativa expirada. Entre novamente.',
